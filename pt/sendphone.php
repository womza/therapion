...
// VALIDATION
if(empty($name) || empty($email) ||  empty($location) ||  empty($phone) ||  empty($question)) {echo "<h3>
A falta de dados</h3><p>Por favor, retorne à página anterior para preencher os campos vazios.</p>";
}
elseif(!ereg("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
	echo "<h3>Problema com o e-mail</h3><p>Por favor, retorne à página anterior para corrigi-lo.</p>";
}
else {
	mail($receiverMail,$psicologo['nombres_apellidos'].": PT: TELEFONE: $location",  $msgformat, "From: $name <$email>");
