...
$phone          = ltrim(rtrim(strip_tags(stripslashes($_POST['phonenumber']))));
...
// VALIDATION
if(empty($name) || empty($email) ||  empty($location) ||  empty($phone) ||  empty($question)) {echo "<h3>Data missing</h3><p>Please return to the form.</p>";
}
elseif(!ereg("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
	echo "<h3>Problem with the e-mail address</h3><p>Please return to the form.</p>";
}
else {
	mail($receiverMail,$psicologo['nombres_apellidos'].": PHONE: $location",  $msgformat, "From: $name <$email>");
