...
// VALIDATION
if(empty($name) || empty($email) ||  empty($location) ||  empty($program) ||  empty($username) ||  empty($question)) {echo "<h3>Les données manquantes</h3><p>S'il vous plaît retourner à la page précédente.</p>";
}
elseif(!ereg("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
	echo "<h3>Problème avec le email.</h3><p>S'il vous plaît retourner à la page précédente.</p>";
}
else {
	mail($receiverMail,$psicologo['nombres_apellidos'].": FR: CHAT: $location",  $msgformat, "From: $name <$email>");
