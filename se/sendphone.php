...
if(empty($name) || empty($email) ||  empty($location) || empty($phone) ||  empty($question)) {
	echo "<h3>Saknande dator</h3><p>Var vänligt och gå tillbaka för att fylla i formulären.</p>";
}
elseif(!ereg("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
	echo "<h3>Fel i din e-postadress</h3><p>Var vänligt och gå tillbaka för att korrigera din e-postadress.</p>";
}
else {
