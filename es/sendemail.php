...
if(empty($name) || empty($email) || empty($location) ||  empty($question)) {
	echo "<h3>Falta de datos</h3><p>Por favor, regrese en la página anterior para llenar los datos.</p>";
}
elseif(!ereg("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
	...
	mail($receiverMail,$psicologo['nombres_apellidos'].": ES: CORREO ELECTRONICO: $location",  $msgformat, "From: $name <$email>");
