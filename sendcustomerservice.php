...
$msgformat	= "From: $name ($ip)\nEmail: $email\nLocation: $location\n\nQuestion:\n$question"; $thankspage="confirmationcustomerservice.php";

// VALIDATION
if(empty($name) || empty($email) || empty($location) ||  empty($question)) {
	echo "<h3>Missing data</h3><p>Please return to the form.</p>";
}
elseif(!ereg("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
	echo "<h3>Problem with the e-mail address</h3><p>Please return to the form.</p>";
}
else {
	mail($receiverMail,"GLOBAL: CUSTOMER SERVICE: $location",  $msgformat, "From: $name <$email>");
